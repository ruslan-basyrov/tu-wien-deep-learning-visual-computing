...
def make_run_name(params: dict) -> str:
    readable = f"yourcnn_{params['augmentation']}_wd{params['weight_decay']}_dropout{params['dropout']}"
...
def train(args):
    ...
    for augmentation, weight_decay, dropout in product(*grid.values()):
        ...
        model = DeepClassifier(YourCNN(dropout_p=dropout))
        model.to(device)

        optimizer = torch.optim.AdamW(
            model.parameters(), lr=0.001, weight_decay=weight_decay, amsgrad=True
        )
...
if __name__ == "__main__":
    args = argparse.ArgumentParser(description="Train Your CNN on CIFAR-10")
    args.add_argument(
